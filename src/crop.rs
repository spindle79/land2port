...
pub fn calculate_two_heads_crop(
    ...
    if bbox.width <= frame_height * 0.75 {
        ...
        let crop2_y = if head1_bottom > frame_height * (17.0 / 18.0)
            || head2_bottom > frame_height * (17.0 / 18.0)
        {
            frame_height / 9.0
        } else {
            default_y
        ...
        let head1_in_crop1 =
            (head1.xmax().min(crop1_x + crop_width) - head1.xmin().max(crop1_x)).max(0.0);
        let head1_in_crop2 =
            (head1.xmax().min(crop2_x + crop_width) - head1.xmin().max(crop2_x)).max(0.0);
        let head2_in_crop1 =
            (head2.xmax().min(crop1_x + crop_width) - head2.xmin().max(crop1_x)).max(0.0);
        let head2_in_crop2 =
            (head2.xmax().min(crop2_x + crop_width) - head2.xmin().max(crop2_x)).max(0.0);

        // Check if either head spans both crops
        let head1_spans = head1_in_crop1 > 0.0 && head1_in_crop2 > 0.0;
        let head2_spans = head2_in_crop1 > 0.0 && head2_in_crop2 > 0.0;

        if head1_spans || head2_spans {
            // Use left/right head positioning approach when heads span crops
        ...
        // Otherwise, keep the default positions (crop1 at x=0, crop2 at x=frame_width-crop_width)
...
pub fn calculate_three_to_five_heads_crop(
    use_stack_crop: bool,
    frame_width: f32,
    frame_height: f32,
    heads: &[&Hbb],
...
pub fn calculate_six_or_more_heads_crop(
    ...
    if bbox.width <= frame_height * (3.0 / 4.0) {
        ...
        for (i, &area) in head_areas.iter().enumerate() {
            let is_large = head_areas.iter().enumerate().all(|(j, &other_area)| {
                i == j || area >= other_area * 2.5
            });
            if is_large {
        ...
        if let Some(large_head_idx) = large_head_index {
            ...
            if use_stack_crop {
                // Create two stacked crops: one centered on the large head, one for remaining heads
                let crop_height = frame_height * (8.0 / 9.0);
                let crop_width = frame_height;
                let crop_y = frame_height / 18.0;
                ...
                if remaining_heads.is_empty() {
                    // If no remaining heads, just return the single crop
                ...
                // Calculate bounding box of remaining heads
                let remaining_bbox = calculate_bounding_box(&remaining_heads);

                // Position second crop to contain remaining heads
                let mut crop2_x = remaining_bbox.x + remaining_bbox.width / 2.0 - crop_width / 2.0;
                crop2_x = crop2_x.max(0.0).min(frame_width - crop_width);
...
pub fn calculate_crop_area(
    ...
    match heads.len() {
        ...
        3..=5 => Ok(calculate_three_to_five_heads_crop(
            use_stack_crop,
            frame_width,
            frame_height,
            heads,
...
mod tests {
    ...
    fn test_is_crop_similar_real_values() {
        ...
        let crop1 = CropResult::Stacked(CropArea::new(32.520447, 60.0, 1080.0, 960.0), CropArea::new(790.54004, 60.0, 1080.0, 960.0));
        let crop2 = CropResult::Stacked(CropArea::new(0.0, 60.0, 1080.0, 960.0), CropArea::new(840.0, 60.0, 1080.0, 960.0));
        assert!(is_crop_similar(&crop1, &crop2, frame_width, threshold));
    }
